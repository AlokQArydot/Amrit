...
public class Frequency2_read extends Login {
	...
	public void Frequency_Report() throws InterruptedException {
		...
		for (int i = 13; i < gatewayOptions.size(); i++) { // Construct the XPath for each item based onits index
			...
			for (int j = 1; j < MeterOptions.size(); j++) {
				...
				try {
					...
					Reporter.log("FrequencyValue is displayed: " + frequencyValue, true);
					...
					if (frequency == 0) {
						...
						Reporter.log("Gateway Name ==" + gatewayName + ",Meter Name ----" + metername +  "The frequency value is zero. Please check.");
					}
	...
	private void handleFrequencyNotPresent(String gatewayName, String metername) {
		try {
			...
			sendEmailToManager("Gateway Name ==" + gatewayName + ",Meter Name ----" + metername + "Text Found is "+tablePText+ "");
